(() => {
  ...
  if ($("#logoImg")) $("#logoImg").src = (window.PRODUCT_IMAGES||{}).logo || "";
  if ($("#coverImg")) $("#coverImg").src = (window.PRODUCT_IMAGES||{}).cover || "";
  ...
  function applyLanguage(){
    ...
    $("#footerAbout").textContent = t("Useful products, straightforward prices, and delivery across Lebanon.","منتجات مفيدة، أسعار واضحة، وتوصيل إلى جميع أنحاء لبنان.");
  ...
  function renderCategories(){
    $("#categories").innerHTML = cats.map(([key,en,ar]) => `<button class="chip ${key===category?"active":""}" data-cat="${key}">${t(en,ar)}</button>`).join("");
    $$("#categories .chip").forEach(b => b.onclick = () => { category = b.dataset.cat; renderCategories(); renderProducts(); });
  }

  function renderProducts(){
    const q = search.trim().toLowerCase();
    const list = P.filter(p => { const catOk = category === "all" || p.category === category; const hay = [p.code,p.name_en,p.name_ar,p.desc_en,p.desc_ar].join(" ").toLowerCase(); return catOk && (!q || hay.includes(q)); });
    $("#productsGrid").innerHTML = list.length ? list.map(p => `
      <article class="product-card">
        <div class="image-wrap">
          <img src="${(window.PRODUCT_IMAGES||{})[p.code]||''}" alt="${lang==="ar"?p.name_ar:p.name_en}" loading="lazy">
        ...
        <div class="product-body">
          ...
          <div class="price-row"><span class="price">${money(p.price)}</span><button class="add-btn" data-add="${p.code}">${t("Add to bag","أضف للسلة")}</button></div>
        </div>
      </article>`).join("") : `<div class="empty">${t("No products found.","لم يتم العثور على منتجات.")}</div>`;
  ...
  function buildOrderText(){
    const name = $("#name").value.trim(), phone = $("#phone").value.trim(), area = $("#area").value.trim(), address = $("#address").value.trim(), notes = $("#notes").value.trim();
    const lines = Object.entries(cart).map(([code,qty])=>{ const p = prodByCode(code); return `${qty} × ${p.name_en} (${code}) — ${money(p.price*qty)}`; });
    return ["🛍️ NEW KTIR RKHIS ORDER","",`Customer: ${name}`,`Phone: ${phone}`,`Area: ${area}`,`Address: ${address || "-"}`,`Notes: ${notes || "-"}`,"","Items:",...lines.map(x=>"• "+x),"",`TOTAL: ${money(cartTotal())}`,"","Delivery: Lebanon"].join("\n");
  }
  ...
  $("#cartBtn").onclick=openCart; $("#closeCart").onclick=closeCart; $("#overlay").onclick=()=>{closeCart();closeCheckout()}; $("#checkoutBtn").onclick=openCheckout; $("#closeCheckout").onclick=closeCheckout; $("#copyOrderBtn").onclick=copyOrder; $("#sendOrderBtn").onclick=sendOrder;
